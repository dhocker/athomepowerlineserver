...
if __name__ == "__main__":
  ...
  if args.port is not None:
    ahps_client.Port = args.port

  # Try a status request command
  response = ahps_client.StatusRequest()
  ...
  response = None
  if cmd == "applianceon":
    response = ahps_client.DeviceOn(args.housedevicecode, 0)
  elif cmd == "lampon":
    response = ahps_client.DeviceOn(args.housedevicecode, args.dimamount)
  elif cmd == "applianceoff" or cmd == "deviceoff":
    response = ahps_client.DeviceOff(args.housedevicecode, 0)
  elif cmd == "lampoff":
