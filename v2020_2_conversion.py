#
# AtHomePowerlineServer - database conversion from v2020_1 to v2020_2
...
def update_managed_devices():
    ...
    conn.execute(
        ...
        mfg text, address text, channel integer, color text, brightness integer, updatetime timestamp)")
    # Copy all rows from ManagedDevices. Default color to #ffffff (white), brightness to 100.
    conn.execute(
        "INSERT INTO DevicesTemp(id,name,location,mfg,address,channel,color,brightness,updatetime) \
        SELECT id,name,location,mfg,address,channel,\"#ffffff\",100,updatetime from ManagedDevices")
...
def update_programs():
    ...
    # New Programs table without dimamount and args and with color and brightness
    conn.execute(
        "CREATE TABLE ProgramsTemp ( \
        id	integer, \
        name	text, \
        daymask	text, \
        triggermethod	text, \
    ...
    # Populate the temp table from the existing Programs table. Default color #FFFFFF and brightness 100.
    conn.execute(
        "INSERT INTO ProgramsTemp(id,name,daymask,triggermethod,time,offset,randomize,randomizeamount,command,color,brightness,updatetime) \
        SELECT id,name,daymask,triggermethod,time,offset,randomize,randomizeamount,command,\"#ffffff\",100,updatetime from Programs")
    conn.commit()

    # Delete Programs table
    conn.execute("DROP TABLE Programs")
    # Rename temp table to Programs
    conn.execute("ALTER TABLE ProgramsTemp RENAME TO Programs")
...
def main():
    next_schema_version = "5.2.0.0"
    current_schema_version = get_schema_version()
    if current_schema_version == "5.1.0.0":
        # Add channel column to ManagedDevices
        update_managed_devices()
        # Replace dimamount and args with color and brightness
        update_programs()
        # To 5.2.0.0
...
if __name__ == "__main__":
    import sys
    print("AtHomePowerlineServer Database conversion from version 5.1.0.0 to 5.2.0.0")
    ...
    print("Converting %s to 5.2.0.0" % db)
    main()
